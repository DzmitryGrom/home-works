...
$(document).ready(function () {
    ...
    ymaps.ready(function () {
        var myMap = new ymaps.Map('map', {
            ...
            ButtonLayout = ymaps.templateLayoutFactory.createClass("<div class=map-pop-up><h2>{{data.header}}</h2><p><i class='fa fa-map-marker'></i><a href='#' class='btn-open-top-panel' onclick='$(\".top-panel\").css(\"transform\", \"translateY(0%)\")'><span class='map-pop-up-text-bottom-city'>{{data.city}}</span><i class='navbar-header-top_box-location-text fa fa-angle-down'></i></a>{{data.strit}}</p><p><span class='map-pop-up-text-bottom-number'><i class='fa fa-phone'></i>{{data.phoneOne}}</span><span class='map-pop-up-text-bottom-number'>{{data.phoneTwo}}</span></p><p><span><i class='fa fa-envelope'></i>{{data.mail}}</span></p><p><span><i class='fa fa-calendar-check-o'></i>{{data.work}}</span></p><p><span><i class='fa fa-clock-o'></i>{{data.time}}</span></p></div>"),
            ...
            placemark = new ymaps.Placemark([56.315695, 44.017063], {
        ...
        myMap.controls.add(button, { position: { left: 0, top: 0 } });
    });
